...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Trim history to the last 3 turns (each turn has user and assistant messages)
        # 3 turns = 6 messages
        if len(history) > 6:
            history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = f"{'Prompt':<40} | {'GPT-4o':<40} | {'Mini':<40} | {'4o Lat':<8} | {'Mini Lat':<8}"
    separator = "-" * len(header)
    lines = [header, separator]
    
    for res in results:
        p = (res["prompt"][:37] + "...") if len(res["prompt"]) > 40 else res["prompt"]
        g = (res["gpt4o_response"][:37] + "...") if len(res["gpt4o_response"]) > 40 else res["gpt4o_response"]
        m = (res["mini_response"][:37] + "...") if len(res["mini_response"]) > 40 else res["mini_response"]
        g_l = f"{res['gpt4o_latency']:.2f}s"
        m_l = f"{res['mini_latency']:.2f}s"
        
        # Handling multiline responses for formatting - just taking first line for simplicity as hinted
        p = p.replace('\n', ' ')
        g = g.replace('\n', ' ')
        m = m.replace('\n', ' ')
        
        row = f"{p:<40} | {g:<40} | {m:<40} | {g_l:<8} | {m_l:<8}"
        lines.append(row)
